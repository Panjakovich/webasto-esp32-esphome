...
- **полностью автономным циклом работы** (розжиг → работа → охлаждение → idle),
- **интеграцией в Home Assistant**,
- **автоматическим выбором мощности в 5 ступеней** по нагрузке и температуре воздуха,
- **управлением по температуре воздуха в помещении** с учётом уличной температуры,
- **PWM‑управлением вентилятором и водяной помпой**,
- **байпас‑клапаном** для быстрого набора температуры,
- продуманной **системой защит** (перегрев, срыв пламени, ошибка датчика, отсутствие роста температуры, лимит неудачных пусков).
...
- **Полный автономный цикл**:
  ...
  - переход в режим `idle` без выключения мастер‑свитча и последующие автозапуски по температуре в комнате.

- **Интеграция в Home Assistant**:
  - `Webasto Master Switch` — включение/выключение котла,
  - `Target Power` (`Very Low` / `Low` / `Medium` / `High` / `Turbo`) — целевая мощность (5 ступеней),
  ...
  - комнатная температура (`room_temp`) и уличная температура (`outdoor_temp`),
  - настраиваемые пороги как `number:`‑сущности (`room_target_temp`, `radiator_target_temp`, пороги ΔT, пороги байпаса и т.д.).

- **Автоматическое управление мощностью**:
  - два датчика DS18B20: на подаче и обратке радиаторного контура,
  - расчёт `ΔT = Flow - Return` и поддержание подачи около целевого значения (по умолчанию ~50 °C, но настраивается),
  - 5 ступеней мощности (`Very Low` / `Low` / `Medium` / `High` / `Turbo`), которые переключаются по:
    - ΔT и температуре подачи,
    - ошибке по температуре воздуха и тренду её изменения (учёт тепловой инерции),
    - уличной температуре (погодозависимая поправка),
  - защита от сажи: ограничение длительной работы в минимальных режимах (`Very Low`/`Low`) с периодическими «прожигами» на более высокой мощности.

- **PWM‑управление помпой и вентилятором**:
  - вентилятор Webasto — 20 кГц PWM (тихая работа),
  - водяная помпа — PWM‑управление, уровень зависит от выбранной ступени мощности (от `Very Low` до `Turbo`).
...
Все силовые нагрузки работают от 12 В. Управление реализовано через N‑канальные MOSFET с драйвером и оптопарой (по схеме на рисунке `Shem.png`):

- **U11 UCC27524** — двухканальный драйвер затворов MOSFET, питается от 5 В и развязывает ESP32 от больших токов по затворам.
- **Q5/Q6/Q3 IRLB3034** — силовые MOSFETы на свечу, вентилятор и топливный насос (низкий Rds(on), большой допустимый ток, обязательны радиаторы для свечи).
- **U9 PC817C** — оптопара для управления ШИМ помпы (развязка логики и силовой части по этому каналу).
- **Защитные диоды 1N5822** (Schottky) параллельно индуктивным нагрузкам — гасят выбросы при выключении.
- **Датчик тока HACS712‑20A** — стоит в разрыв силовой шины и даёт сигнал тока (после делителя) на вход ESP32 для контроля пламени.

Рекомендуется дополнительно:

- ставить резисторы затвор‑исток (47–100 кОм) на каждый MOSFET, чтобы гарантированно закрывать его при старте/обрыве сигнала,
- использовать резисторы в затворах порядка 10–15 Ω для снижения звона и ЭМИ,
- добавить на входе питания TVS‑диод (например, SMBJ24A) и электролитический конденсатор 1000–2200 мкФ максимально близко к силовым MOSFET.
